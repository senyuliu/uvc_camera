...
namespace uvc_camera {
    ...
    cv::Mat Array2Mat(uchar *array, int row, int col) {
        // Mat (int rows, int cols, int type, void *data, size_t step=AUTO_STEP)
        cv::Mat img(row, col, CV_8UC3, (unsigned char *)array);
        return img;
    }
...
Camera::Camera(ros::NodeHandle _comm_nh, ros::NodeHandle _param_nh) :
    ...
    void Camera::feedImages() {
      ...
      cv::VideoWriter mp4Writer;
      std::string file_out = "/home/video0.mp4";
      mp4Writer.open(file_out , mp4Writer.fourcc('M','J','P','G'), 30 , cv::Size(width, height),1);
      ...
      while (ok) {
        ...
        if (skip_frames == 0 || frames_to_skip == 0) {
          if (img_frame && format != "jpeg") {
             ...
             cv::Mat matFrame = Array2Mat(img_frame, height, width);
             //cv::imwrite("test.jpg", matFrame);
